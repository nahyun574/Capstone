...
def media():
    ...
    #캠 사이즈
    newsize = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    edges = cv2.resize(edges,newsize)
        ...
        def media():
            ...
            with mp_pose.Pose(
                ...
                while cap.isOpened():
                    ...
                    show_fps.ShowFPS(frame,last_time,time_per_frame_video)
                    if cv2.waitKey(5) & 0xFF == 27:
                        break
            cap.release()
        '''
